...
_CONSTEXPR14 inline bool Int128MultiplyInt64( int64_t a, int64_t b, int64_t* pRet ) SAFEINT_NOTHROW
{
    ...
    const int64_t resultLow = static_cast<int64_t>(result);
    *pRet = resultLow;

    // The upper 65 bits must have the same value, else overflow.
...
_CONSTEXPR14 inline bool Int128MultiplyUint64( uint64_t a, uint64_t b, uint64_t* pRet ) SAFEINT_NOTHROW
{
        ...
        *pRet = static_cast<uint64_t>(result);

        // The upper 64bits must be zero, else overflow.
        return resultHigh == 0;
}
...
public:
...
#elif SAFEINT_USE_INT128
        return Int128MultiplyUint64( a, b, pRet );
#else
    ...
    _CONSTEXPR14_MULTIPLY static void RegMultiplyThrow( const std::uint64_t& a, const std::uint64_t& b, std::uint64_t* pRet ) SAFEINT_CPP_THROW
    {
        ...
        if( !IntrinsicMultiplyUint64( a, b, pRet ) )
            E::SafeIntOnOverflow();
#elif SAFEINT_USE_INT128
        if ( !Int128MultiplyUint64( a, b, pRet ) )
            E::SafeIntOnOverflow();
#else
        std::uint32_t aHigh = 0, aLow = 0, bHigh = 0, bLow = 0;
...
public:
...
#elif SAFEINT_USE_INT128
        return Int128MultiplyUint64( a, (std::uint64_t)b, pRet );
#else
    ...
    _CONSTEXPR14_MULTIPLY static void RegMultiplyThrow( const std::uint64_t& a, std::uint32_t b, std::uint64_t* pRet ) SAFEINT_CPP_THROW
    {
        ...
        if( !IntrinsicMultiplyUint64( a, (std::uint64_t)b, pRet ) )
            E::SafeIntOnOverflow();
#elif SAFEINT_USE_INT128
        if( !Int128MultiplyUint64( a, (std::uint64_t)b, pRet ) )
            E::SafeIntOnOverflow();
#else
        std::uint32_t aHigh = 0, aLow = 0;
...
public:
...
#elif SAFEINT_USE_INT128
        return Int128MultiplyInt64( a, b, pRet );
#else
    ...
    _CONSTEXPR14_MULTIPLY static void RegMultiplyThrow( const std::int64_t& a, const std::int64_t& b, std::int64_t* pRet ) SAFEINT_CPP_THROW
    {
        ...
        if( !IntrinsicMultiplyInt64( a, b, pRet ) )
            E::SafeIntOnOverflow();
#elif SAFEINT_USE_INT128
        if ( !Int128MultiplyInt64( a, b, pRet ) )
            E::SafeIntOnOverflow();
#else
        bool aNegative = false;
        bool bNegative = false;
...
public:
    ...
    _CONSTEXPR14_MULTIPLY static void RegMultiplyThrow( const std::int64_t& a, std::uint32_t b, std::int64_t* pRet ) SAFEINT_CPP_THROW
    {
        ...
        if( !IntrinsicMultiplyInt64( a, (std::int64_t)b, pRet ) )
            E::SafeIntOnOverflow();
#elif SAFEINT_USE_INT128
        if( !Int128MultiplyInt64( a, (std::int64_t)b, pRet ) )
            E::SafeIntOnOverflow();
#else
        bool aNegative = false;
        std::uint64_t tmp = 0;
